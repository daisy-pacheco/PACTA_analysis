...
port_weight_cb_input <- result_data_qa %>% 
  ...
  summarise(weight_value_usd = round(sum(value_usd, na.rm = T)/max(total_value_usd), 9)) %>% 
  ungroup() %>% 
...
port_weight_cb_output <- bonds_results_company %>% 
  ...
  summarise(max_port_weight = round(max(port_weight), 9)) %>% 
  ungroup() %>% 
...
port_weight_cb_comp <- port_weight_cb_input %>% 
  ...
  arrange(financial_sector, corporate_bond_ticker) %>% 
  mutate(weight_diff = round((max_port_weight - weight_value_usd), 9))
